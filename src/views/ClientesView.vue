<script setup>
    import { onMounted, ref, computed } from 'vue';
    import ClienteService from '../services/ClienteService.js';
    import RouterLink from '../components/UI/RouterLink.vue';
    import Heading from '../components/UI/HedingTe.vue';
    import Cliente from '../components/Cliente.vue';   /* PARA LLAMAR A LA API */

    const clientes = ref([])

    onMounted(() =>{
        ClienteService.obtenerClientes()
            .then(({data}) => clientes.value = data)
            .catch(error => console.log('hubi un error'))
    })

    defineProps({
        titulo: {
            type: String
        }
    })

    /* validamos si existen clientes  */
    const existeClientes = computed(() => {
        return clientes.value.length > 0
    })

    /* para eliminar un registro */
    const eliminarCliente = id => {
        ClienteService.eliminarCliente(id)
            .then(() => {
                /* para que elimine y recarge */
                clientes.value = clientes.value.filter(clientes => clientes.id !== id)
            })
            .catch(error => console.log(error))
    }
</script>


<template>
    <div>
        <div class="flex justify-end">
            <RouterLink to="agregar">
                Agregar Usuario
            </RouterLink>
        </div>
        <Heading>Lista de Usuarios</Heading>
        <!-- <Heading>{{ titulo }}</Heading> -->

      
        <div v-if="existeClientes" class="flow-root mx-auto  mt-10 p-5 bg-white shadow">
            <div class="-my-2 -mx-4 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="min-w-full py-2 align-middle sm:px-6 lg:px-8">
                    <table class="min-w-full divide-y divide-gray-300">
                        <thead>
                        <tr>
                            <th scope="col" class="p-2 text-left text-sm font-extrabold text-gray-600">Nombre</th>
                            <th scope="col" class="p-2 text-left text-sm font-extrabold text-gray-600">Empresa</th>
                            <th scope="col" class="p-2 text-left text-sm font-extrabold text-gray-600">Acciones</th>
                        </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-200 bg-white">
                                <Cliente 
                                    v-for="cliente in clientes" 
                                    :key="cliente.id"
                                    :cliente="cliente"
                                    @eliminarCliente="eliminarCliente"
                                    />
                        </tbody>
                            
                    </table>
                </div>
            </div>
        </div>

        
        <p v-else class="text-center mt-10"> No ahi clientes</p>

        
    </div>
  
</template>


